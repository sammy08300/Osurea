    ...
    if (!translated || translated.startsWith('i18n:')) {
        ...
        const fallbackTranslations = {
            'select_model': {
                ...
                'fr': 'SÃ©lectionner un modÃ¨le'
            },
            'custom_dimensions': {
                'en': 'Custom dimensions',
                'es': 'Dimensiones personalizadas',
                'fr': 'Dimensions personnalisÃ©es'
            },
            'no_results': {
                'en': 'No results found',
...
const TabletSelector = {
    ...
    init(tabletData) {
        ...
        // Ã‰viter d'afficher "SÃ©lectionner un modÃ¨le" si on a des prÃ©fÃ©rences sauvegardÃ©es
        let savedPrefs = null;
        try {
        ...
        if (savedPrefs && savedPrefs.tablet && savedPrefs.tablet.brand && savedPrefs.tablet.model) {
            // DÃ©terminer le texte Ã  afficher en vÃ©rifiant si le modÃ¨le inclut dÃ©jÃ  la marque
            let displayText = '';
            if (savedPrefs.tablet.model.includes(savedPrefs.tablet.brand)) {
                // Si le modÃ¨le contient dÃ©jÃ  la marque, l'utiliser tel quel
                displayText = savedPrefs.tablet.model;
            } else {
                // Sinon, afficher la marque suivie du modÃ¨le
            ...
            // Si pas de prÃ©fÃ©rences, utiliser la traduction mais de faÃ§on optimisÃ©e
            // pour Ã©viter le flash du texte par dÃ©faut
            ...
            // Pour les nouveaux utilisateurs, charger la tablette par dÃ©faut immÃ©diatement
        ...
        // VÃ©rifier Ã  nouveau si aucune tablette n'a Ã©tÃ© chargÃ©e
        // Ce timeout est maintenu pour s'assurer que toute l'initialisation est terminÃ©e
        setTimeout(() => {
    ...
    loadTabletFromPreferences(tabletData) {
        ...
        this.loadedFromPreferences = true; // Marquer comme chargÃ© depuis les prÃ©fÃ©rences
        
        // Stockage immÃ©diat des donnÃ©es sur le bouton pour Ã©viter le flash
        ...
        if (tabletData.isCustom) {
            ...
            // Ã‰mettre un Ã©vÃ©nement pour confirmer le chargement
        ...
        if (tablet) {
            ...
            if (isValidNumber(tabletData.width) && isValidNumber(tabletData.height)) {
                ...
                // Ã‰mettre un Ã©vÃ©nement pour signaler l'Ã©chec
                document.dispatchEvent(new CustomEvent('tablet:loaded-from-preferences', { 
    ...
    loadDefaultTablet() {
        ...
        if (defaultTablet) {
            ...
            if (this.tabletData.length > 0) {
                ...
                console.error('Aucun modÃ¨le de tablette disponible!');
            }
        }
    },
    ...
    populateBrandsList() {
        ...
        if (!this.tabletData || this.tabletData.length === 0) {
            ...
            noData.textContent = 'Aucune donnÃ©e de tablette disponible';
    ...
    displayModels(models) {
        ...
        if (models.length === 0) {
            ...
            noModels.textContent = 'Aucun modÃ¨le disponible';
    ...
    selectModel(tablet) {
        ...
        // VÃ©rifier si le modÃ¨le inclut dÃ©jÃ  la marque pour Ã©viter la duplication
        let displayText = '';
        if (tablet.model.includes(tablet.brand)) {
            // Si le nom du modÃ¨le contient dÃ©jÃ  la marque, l'utiliser tel quel
